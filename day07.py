...
for rule in rules:
    ...
    for color_and_number in bags_inside_container.split(","):
        if not color_and_number.startswith(" no other"):
            color_bag = color_and_number[2:color_and_number.index("bag")].strip(" ")
            number_of_bags = int(color_and_number[0:2].strip(" "))
...
def bags_can_contain(color, color_set=set()):
    ...
    if color in colors_map.keys():
        for c in colors_map.get(color):
            bags_can_contain(c, color_set)
    return color_set
