...
def compute_simple_expression(expression):
    ...
    while i < len(terms):
        ...
        if terms[i].startswith("("):
            ...
            pos = expression_pos + 1
            while nb_closing_parenthesis != nb_opening_parenthesis:
                if expression[pos] == "(":
                    nb_opening_parenthesis += 1
                elif expression[pos] == ")":
                    ...
                    i += 1
                pos += 1
            sub_expression = expression[expression_pos+1:pos-1]
            val = compute_simple_expression(sub_expression)
            expression_pos = pos + 1
        else:
            val = int(terms[i])
            expression_pos += len(terms[i]) + 1
