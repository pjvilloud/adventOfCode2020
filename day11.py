...
def seat_empty(grid, x, y):
    return grid[x][y] == 'L'


def seat_occupied(grid, x, y):
    return grid[x][y] == '#'
...
def find_first_visible_seat(grid, x, y, x_factor = 0, y_factor = 0):
    while 0 <= x + x_factor < len(grid) and 0 <= y + y_factor < len(grid[0]) and is_floor(grid, x + x_factor, y + y_factor):
        x += x_factor
        y += y_factor
    return x + x_factor, y + y_factor


def nb_seats_adjacent_occupied(grid, x, y):
    nb_seats_occ = 0
    for coord in [(x - 1, y), (x - 1, y - 1), (x - 1, y + 1), (x, y + 1), (x, y - 1), (x + 1, y), (x + 1, y + 1), (x + 1, y - 1)]:
        if 0 <= coord[0] < len(grid) and 0 <= coord[1] < len(grid[0]) and seat_occupied(grid, coord[0], coord[1]):
            nb_seats_occ += 1
    return nb_seats_occ

def nb_seats_adjacent_occupied_v2(grid, x, y):
    nb_seats_occ = 0
    coords = [
        find_first_visible_seat(grid, x, y, -1),
        find_first_visible_seat(grid, x, y, -1, -1),
        find_first_visible_seat(grid, x, y, -1, 1),
        find_first_visible_seat(grid, x, y, 0, 1),
        find_first_visible_seat(grid, x, y, 0, -1),
        find_first_visible_seat(grid, x, y, 1),
        find_first_visible_seat(grid, x, y, 1, 1),
        find_first_visible_seat(grid, x, y, 1, -1)
...
def nb_occupied_seats(grid):
    nb_seats = 0
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if seat_occupied(grid, i, j):
                nb_seats += 1
    return nb_seats


def apply_round(grid):
    grid_after_rules = copy.deepcopy(grid)
    # Each line
    for i in range(len(grid)):
        # Each position
        for j in range(len(grid[i])):
            if seat_empty(grid, i, j) and nb_seats_adjacent_occupied(grid, i, j) == 0:
                grid_after_rules[i][j] = "#"
            elif seat_occupied(grid, i, j) and nb_seats_adjacent_occupied(grid, i, j) >= 4:
                grid_after_rules[i][j] = "L"
    return grid_after_rules


def apply_round_v2(grid, v2=False):
    grid_after_rules = copy.deepcopy(grid)
    # Each line
    for i in range(len(grid)):
        # Each position
        for j in range(len(grid[i])):
            if seat_empty(grid, i, j) and nb_seats_adjacent_occupied_v2(grid, i, j) == 0:
                grid_after_rules[i][j] = "#"
            elif seat_occupied(grid, i, j) and nb_seats_adjacent_occupied_v2(grid, i, j) >= 5:
                grid_after_rules[i][j] = "L"
    return grid_after_rules


ferry_grid = map(list, input.splitlines())
ferry_grid_v2 = copy.deepcopy(ferry_grid)

while 1:
    ferry_grid_after_round = apply_round(ferry_grid)
    if ferry_grid_after_round == ferry_grid:
        break
    ferry_grid = ferry_grid_after_round

while 1:
    ferry_grid_after_round = apply_round_v2(ferry_grid_v2)
    if ferry_grid_after_round == ferry_grid_v2:
        break
    ferry_grid_v2 = ferry_grid_after_round

display_grid(ferry_grid)
print "The number of occupied seats is", nb_occupied_seats(ferry_grid)
print "The number of occupied seats in v2 is", nb_occupied_seats(ferry_grid_v2)
